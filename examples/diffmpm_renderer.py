...
def buffers():
    ...
    ti.root.dense(ti.ijk,
                  4).dense(ti.ijk, particle_grid_res // 8).pointer().dense(
                      ti.ijk, 8).dynamic(ti.l,
                                         max_num_particles_per_cell).place(pid)
...
def inside_particle_grid(ipos):
    ...
    return bbox[0][0] <= ipos[0] * dx and ipos[
        0] < bbox[1][0] * inv_dx and bbox[0][1] * inv_dx <= ipos[1] and ipos[
            1] < bbox[1][1] * inv_dx and bbox[0][2] * inv_dx <= ipos[
                2] and ipos[2] < bbox[1][2] * inv_dx
...
def main():
    sand = np.fromfile("mpm3d/iter{}/{:04d}.bin".format(folder, frame_id),
                       dtype=np.float32)
    ...
    for i in range(3):
        ...
        bbox[0][i] = (math.floor(sand[:, i].min() * particle_grid_res) -
                      3.0) / particle_grid_res
        bbox[1][i] = (math.floor(sand[:, i].max() * particle_grid_res) +
                      3.0) / particle_grid_res
