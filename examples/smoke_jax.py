...
def d_simulate(vx, vy, smoke, target, num_time_steps, ax=None, render=False):
    ...
    for t in range(num_time_steps - 1, -1, -1):
        ...
        d_vx_updated, d_vy_updated = d_project(vx_updated, vy_updated, d_vx,
                                               d_vy)
...
def plot_matrix(ax, mat, t, render=False):
    ...
    plt.draw()
    if render:
        matplotlib.image.imsave('output_autograd/step{0:03d}.png'.format(t),
                                mat)
    plt.pause(0.001)
...
if __name__ == '__main__':
    ...
    init_smoke = cv2.resize(imread(os.path.join(basepath, 'init_smoke.png')),
                            (n_grid, n_grid))[:, :, 0]
    ...
    for i in range(10):
        ...
        r = d_simulate(init_vx, init_vy, init_smoke, target,
                       simulation_timesteps)
    ...
    os.system(
        "convert -delay 5 -loop 0 output_autograd/step*.png"
        " -delay 250 output_autograd/step100.png output_autograd/surprise.gif"
    )  # Using imagemagick.
