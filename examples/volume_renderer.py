...
def compute_loss(view_id: ti.i32):
  for i in range(res):
    for j in range(res):
      loss[None] += (images[view_id, i, j] - target_images[view_id, i, j]) ** 2 * (1.0 / (res * res))
...
def create_target_images():
  for view in range(n_views):
    ray_march(target_images, math.pi / n_views * view - math.pi / 2.0, view)
...
def main():
  ...
  for iter in range(num_iterations):
    ...
    with ti.Tape(loss):
      for view in range(n_views):
        ray_march(images, math.pi / n_views * view - math.pi / 2.0, view)
        compute_loss(view)
